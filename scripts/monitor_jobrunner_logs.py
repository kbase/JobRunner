...
def job_contains_unhandled_error():
    ...
    unhandled_error_pattern2 = r"ERROR:jr:Error: An unhandled error was encountered"
    job_finished_pattern = r"INFO:sanic.root:Server Stopped"
    with open(job_runner_error_fp, "r", encoding="utf-8") as f:
        error_log = f.read()
        for item in [unhandled_error_pattern,unhandled_error_pattern2,job_finished_pattern]:
            if item in error_log:
                return True
    return False
